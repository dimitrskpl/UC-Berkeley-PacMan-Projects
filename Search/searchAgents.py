...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #(coordinates, visitedCorners)
        #typecast to tuple
        #in order to be used in set
    ...
    def isGoalState(self, state):
        ...
        if curNode not in visitedCorners and curNode in self.corners:
                visitedCorners.append(curNode)
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            nextState = self.getNextState(state, action)
            child = (nextState, action, self.getActionCost(state, action, nextState))
    ...
    def getNextState(self, state, action):
        ...
        if nextNode not in visitedCorners and nextNode in self.corners:
            childVisitedCorners.append(nextNode)

        #typecast to tuple
        #in order to be used in set
        return (nextNode, tuple(childVisitedCorners))
        util.raiseNotDefined()
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # create a list with the missed corners
    visitedCorners = state[1]
    missedCorners = []
    for corner in corners:
        if corner not in visitedCorners:
            missedCorners.append(corner)

    totalCost = 0
    curPoint = state[0]
    #while there are corners left
    while len(missedCorners):
        cornersDistance = []
        #compute distance for all the missed corners from curPoint
        # using manhattanDistance
        for corner in missedCorners:
            cornersDistance.append((util.manhattanDistance(curPoint, corner), corner))   
        #find the minimum of the costs and the corresponding corner
        heuristicCost, corner = min(cornersDistance) 
        missedCorners.remove(corner)
        curPoint = corner
...
def foodHeuristic(state, problem):
    ...
    # Find distances between position and all of the food 
    distance = []
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        #list with coordinates of food
        foodList = self.food.asList()
        #if Pacman's position coordinates are in the coordinates of food
